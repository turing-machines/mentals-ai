...
* The `Agent Executor` 🧠 is implemented as a recursive loop where the LLM decides what the next loop will be: what instruction (prompt) to execute and what data to pass to that instruction based on data from previous loops. The Agent Executor logic is placed in the system prompt, see [mentals_system.prompt](mentals_system.prompt). This is an important difference.
