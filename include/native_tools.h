...
std::string tool_memory(std::shared_ptr<AgentExecutor> ce_ref, json args) {
  std::string answer;
  assert(args.contains("keyword") &&
         "tool_memory must contain the 'keyword' key.");
  assert(args.contains("content") &&
         "tool_memory must contain the 'content' key.");
  assert(args.contains("description") &&
         "tool_memory must contain the 'description' key.");
  ...
  ce_ref->short_term_memory =
      remove_element_by_name(ce_ref->short_term_memory, "keyword", keyword);
  ce_ref->short_term_memory.push_back({{"keyword", keyword},
...
std::string tool_send_message(std::shared_ptr<AgentExecutor> ce_ref,
                              json args) {
  std::string answer;
  if (args.contains("message")) {
    ...
    std::string answer =
        fmt::format("The message: '{}' was successfully displayed", message);
  }
  return answer;
}
  ...
  if (args.contains("prompt")) {
    if (!debug) {
      std::string completion =
          ce_ref->agent_executor_state["output"].get<std::string>();
      completion = string_in_line(completion);
      completion += "\n";
      stop_spinner(completion);
    }
    ...
    answer = fmt::format("User message is: '{}'", message);
  }
  return answer;
}
...
std::string tool_execute_bash_command(std::shared_ptr<AgentExecutor> ce_ref,
                                      json args) {
  (void)ce_ref;
  std::string answer;
...
std::string tool_execute_python_script(std::shared_ptr<AgentExecutor> ce_ref,
                                       json args) {
  std::string answer;
  if (args.contains("script")) {
    ...
    std::string result =
        ce_ref->code_interpreter.run_python_code(script, dependencies);
    if (debug) {
